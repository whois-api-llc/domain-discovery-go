...
const (
	...
	userAgent      = "domain-discovery-go/" + libraryVersion
	mediaType      = "application/json"
)

// defaultDomainDiscoveryURL is the default Domains & Subdomains Discovery API URL.
const defaultDomainDiscoveryURL = `https://domains-subdomains-discovery.whoisxmlapi.com/api/v1`
...
type ClientParams struct {
	...
	// DomainDiscoveryBaseURL is the endpoint for 'Domains & Subdomains Discovery API' service
	DomainDiscoveryBaseURL *url.URL
}
...
func NewClient(apiKey string, params ClientParams) *Client {
	...
	apiBaseURL := params.DomainDiscoveryBaseURL
	if apiBaseURL == nil {
		apiBaseURL, err = url.Parse(defaultDomainDiscoveryURL)
		if err != nil {
			panic(err)
		}
	}
	...
	client.DomainDiscovery = &domainDiscoveryServiceOp{client: client, baseURL: apiBaseURL}

	return client
}

// Client is the client for Domains & Subdomains Discovery API services.
type Client struct {
	client *http.Client
